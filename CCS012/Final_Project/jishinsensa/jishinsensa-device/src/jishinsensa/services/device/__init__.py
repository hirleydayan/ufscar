            ...
            for sensor_id, sensor_value in iter(queue.get, None):
                ...
                if (timestamp - init_timestamp) >= delay:
                    for pub in publishers:
                        ...
                        __agg_dictionary["timestamp"] = timestamp
                        device_data = json.dumps(
                                         {"dragonboard": __agg_dictionary},
                                         sort_keys=True)
                ...
                sensor_data_list = []
                if sensor_id in __agg_dictionary.keys():
                    sensor_data_list = __agg_dictionary[sensor_id]
                sensor_data_list.append(sensor_data)
                __agg_dictionary[sensor_id] = sensor_data_list
    except KeyboardInterrupt:
        pass
